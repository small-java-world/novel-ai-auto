...
拡張機能はNovelAIの様々なUIバリエーションに対応するため、複数のセレクタープロファイルを提供しています：

**デフォルトプロファイル (`$default`)**

- 基本的なNovelAI UIに対応
- プロンプト入力、ネガティブプロンプト、生成ボタンなどの標準セレクター

**キャラクター別プロファイル**

- `character-anime`: アニメ系キャラクター生成UI
- `character-realistic`: リアル系キャラクター生成UI
- `character-fantasy`: ファンタジー系キャラクター生成UI
- `character-sci-fi`: SF系キャラクター生成UI
- `character-horror`: ホラー系キャラクター生成UI

**UI形式別プロファイル**

- `novelai-mobile`: モバイル表示対応
- `novelai-v3-ui`: NovelAI v3インターフェース
- `novelai-nai-diffusion`: NAI Diffusionモデル専用UI
- `novelai-image-2-image`: Image-to-Image生成UI
- `novelai-inpainting`: インペインティング(塗りつぶし)UI

各プロファイルは `config/dom-selectors.json` で設定されており、UI要素の自動検出と確実な操作を保証します。
