...
const SYNTHESIS_CONFIG = {
    ...
    WARNING_MESSAGES: {
        CHARACTER_LIMIT_EXCEEDED: (current, limit) => `文字数が制限を超過しています: ${current}/${limit}`,
        ...
        SYNTHESIS_TIME_EXCEEDED: (time) => `合成処理時間が制限を超過しました: ${time.toFixed(2)}ms`,
        PREVIEW_TIME_EXCEEDED: (time) => `プレビュー処理時間が制限を超過しました: ${time.toFixed(2)}ms`
    },
    /** 【エラーメッセージ】: システムエラー用のメッセージ */
    ERROR_MESSAGES: {
        INVALID_RULE_ID: (ruleId) => `無効なルールID: ${ruleId}。デフォルトルールを使用します。`,
...
export class PromptSynthesizer {
    ...
    validateResult(result) {
        try {
            ...
            const criticalWarnings = result.warnings.filter(warning => warning.includes('制限を超過') || warning.includes('エラー'));
