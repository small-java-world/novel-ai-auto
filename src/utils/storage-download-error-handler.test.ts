...
import { ERROR_MESSAGES, STORAGE_DOWNLOAD_CONFIG, LOG_LEVELS } from "./storage-download-compatibility-config";
...
describe("DownloadErrorHandler", () => {
  ...
  it("honours explicit retry override", () => {
    const result = DownloadErrorHandler.createErrorResult("PERMISSION_DENIED", new Error("nope"), true);
    expect(result.retryable).toBe(true);
    expect(result.retryDelay).toBeLessThanOrEqual(STORAGE_DOWNLOAD_CONFIG.MAX_RETRY_DELAY);
  });

  it("maps error messages to error types", () => {
    expect(DownloadErrorHandler.classifyError(new Error("permission API error: timeout"))).toBe("PERMISSION_API_ERROR");
    expect(DownloadErrorHandler.classifyError(new Error("Permission denied by user"))).toBe("PERMISSION_DENIED");
    expect(DownloadErrorHandler.classifyError(new Error("invalid request body"))).toBe("INVALID_INPUT");
