...
describe("ensureDownloadPermissionAndDownload", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    errorHandlerMock.createErrorResult.mockReturnValue({ success: false, errorCode: "INVALID_INPUT" });
  ...
  it("logs and returns error when permission request is denied", async () => {
    ...
    errorHandlerMock.createErrorResult.mockReturnValue({ success: false, errorCode: "PERMISSION_DENIED" });
